...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `dobot`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in dobot.__all__ or not hasattr(dobot, obj.name)
    ]
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("dobot")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "dobot" or item.name.startswith("dobot."))
        ):
